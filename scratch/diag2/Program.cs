...
if (Value("--bridge") is not null)
{
    ...
    if (Value("--safe-probe") is not null)
    {
        SafeProbe.Run(
            rom,
            Value("--probe-host") ?? "src/PixelDeck.App/bin/Release/net10.0/PixelDeck.App.exe",
            int.TryParse(Value("--timeout"), out var st) ? st : 20);
        return 0;
    }

    if (Value("--probe-native") is not null)
    {
        Bridge.ProbeNative(rom, int.TryParse(Value("--timeout"), out var t) ? t : 20);
        return 0;
    }

    if (Value("--compare-modes") is not null)
    {
        Bridge.CompareModes(rom, fields);
        return 0;
    }
...
if (Value("--no-dynamic-idle") is not null)
{
    machine.Cpu.DynamicIdleDetectionEnabled = false;
}
