...
The earlier SNES certification run matched all 23 pinned 65C816 result screens exactly, booted all six standard cartridge variants, soaked seven local LoROM/HiROM games for 126,000 frames with continuous audio and exact mid-run state restoration, and published Linux x64 and Linux ARM64 builds. PixelSNES 0.8.009 adds focused DSP-1 protocol and mapping tests plus local Mario Kart progression. Subsequent game-playability testing showed that this evidence was not sufficient for a 1.0 claim, so the product version remains pre-release. This is a bounded compatibility core, not a claim of cycle-perfect S-CPU/PPU or DSP-1B timing. Offset-per-tile details, native 512-pixel high-resolution output, PAL, other enhancement chips, and on-device Raspberry Pi validation remain future work.

Run `./scripts/Test-PixelSnesRelease.ps1` from the repository root. The previous certification attempt, required gates, and evidence are retained in [the historical PixelSNES 1.2 certification record](docs/PIXELSNES-1.2-CERTIFICATION.md). PixelSNES 0.8.009 remains a development build until its full gameplay gates pass.
