...
PixelNES 1.7.011 is the current stable patch build. It retains the 1.7 certification envelope, and PixelDeck now masks the horizontal 8-pixel overscan edges by default so game-generated scrolling artifacts such as Mega Man's title-screen strip are hidden without changing PPU timing or game logic. The full 256-pixel output can be restored in Nintendo settings. Run `./scripts/Test-PixelNesRelease.ps1` from the repository root; the complete contract and baseline evidence are documented in [PixelNES 1.7 certification](docs/PIXELNES-1.7-CERTIFICATION.md).
