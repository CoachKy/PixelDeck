...
public sealed class NesMachineTests
{
    ...
    {
        var gamesFolder = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "Games"));
        if (!Directory.Exists(gamesFolder))
        {
            return;
        }

        var discoveredImages = Directory.EnumerateFiles(gamesFolder, "*.nes", SearchOption.AllDirectories).ToArray();
        ...
        if (discoveredImages.Length > 0)
        {
            ...
            Assert.True(imagesWithAudio > 0, "At least one local NES image should exercise the APU during the smoke test.");
        }
