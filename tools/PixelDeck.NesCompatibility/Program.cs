...
try
{
    ...
    Console.WriteLine("PixelNES compatibility laboratory");
    Console.WriteLine($"Games:    {options.GamesFolder}");
    Console.WriteLine($"Output:   {options.OutputFolder}");
    Console.WriteLine($"Frames:   {options.FramesPerGame}");
    ...
    var runner = new NesCompatibilityRunner();
    var report = await runner.RunAsync(
        ...
        (completed, total, game) =>
        {
            ...
            {
                Console.WriteLine(
                    $"[{completed,4}/{total}] {game.Status,-11} " +
                    $"M{game.Mapper?.ToString() ?? "-",-3} {game.RelativePath}");
            }
        });
    ...
    Console.WriteLine(
        ...
        $"failed {report.Summary.Failed}, unsupported {report.Summary.Unsupported}, " +
